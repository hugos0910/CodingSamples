...
% Abstract   : Homework 7.  This HW assignment solved the steady
%              flowfield properties of a C-D nozzle using MacCormack 
%              method.
%------------------------------------------------------------------%
clc; clear; close all; format long; format compact;

% Inputs
x          = linspace(0,1,51); x = x'; % Nozzle partitions
[m,n]      = size(x);
A          = 1+10*(x-0.5).^2; A = A';  % Nozzle contour
dA         = 20*x-10;                  % Deriv. of the nozzle contour
...
rho = zeros(m,time_iter+1);            % Density
u   = zeros(m,time_iter+1);            % Velocity
p   = zeros(m,time_iter+1);            % Pressure
T   = zeros(m,time_iter+1);            % Temperature
...
dt  = zeros(n,m);
% Initial values
% Inlet boundary conditions (fixed values)
rho(1,:) = 0.985999;
u(1,:)   = 0.183697;
p(1,:)   = 0.980454;
T(1,:)   = p(1,:)./rho(1,:);
...
% Interpolation for properties at interior points
rho_slope = (rho(m,1)-rho(1,1))/(x(m)-x(1)); % Density slope
u_slope = (u(m,1)-u(1,1))/(x(m)-x(1));       % Velocity slope
p_slope = (p(m,1)-p(1,1))/(x(m)-x(1));       % Pressure slope
T_slope = (T(m,1)-T(1,1))/(x(m)-x(1));       % Temperature slope
for i = 2:m
    rho(i,1) = rho_slope*(x(i)-x(1)) + rho(1,1);
    u(i,1) = u_slope*(x(i)-x(1)) + u(1,1);
    p(i,1) = p_slope*(x(i)-x(1)) + p(1,1);
    T(i,1) = T_slope*(x(i)-x(1)) + T(1,1);
end
...
for iter = 1:time_iter
    ...
    % Extrapolation to obtain properties at the exit
    rho_slope = (rho_C(m-2)-rho_C(m-1))/(x(m-2)-x(m-1)); % Density slope
    u_slope = (u_C(m-2)-u_C(m-1))/(x(m-2)-x(m-1)); % Velocity slope
    p_slope = (p_C(m-2)-p_C(m-1))/(x(m-2)-x(m-1)); % Pressure slope
   
    rho_C(m) = rho_slope*(x(m)-x(m-1)) + rho_C(m-1);
    u_C(m) = u_slope*(x(m)-x(m-1)) + u_C(m-1);
    p_C(m) = p_slope*(x(m)-x(m-1)) + p_C(m-1);
